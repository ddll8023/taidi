...
def _parse_stock_research_df(df: pd.DataFrame, source_label: str = "个股研报") -> List[Dict]:
    required_columns = ["title", "stockCode", "stockName", "orgName", "publishDate"]
    missing_columns = [col for col in required_columns if col not in df.columns]
    if missing_columns:
        raise ValueError(f"{source_label}缺少必需字段: {missing_columns}")
...
def _parse_industry_research_df(df: pd.DataFrame, source_label: str = "行业研报") -> List[Dict]:
...
def _normalize_publish_date(publish_date) -> str:
...
def parse_stock_research_excel(file_path: str) -> List[Dict]:
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"个股研报文件不存在: {file_path}")
...
def parse_industry_research_excel(file_path: str) -> List[Dict]:
    if not os.path.exists(file_path):
        raise FileNotFoundError(f"行业研报文件不存在: {file_path}")
...
def parse_stock_research_excel_from_upload(file_content: bytes) -> Tuple[List[Dict], List[str]]:
...
def parse_industry_research_excel_from_upload(file_content: bytes) -> Tuple[List[Dict], List[str]]:
...
def parse_fujian5_excel_data(
    ...
    Args:
        fujian5_dir: 附件5数据目录,默认使用 config.FUJIAN5_DIR

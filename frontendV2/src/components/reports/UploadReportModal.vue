...
/**
 * 上传财报 PDF 弹窗
 * 功能描述：支持批量选择 PDF 文件，逐文件上传到后端并展示结果
 * 依赖组件：无
 */
...
const completedCount = computed(
  () => uploadProgress.value.filter((p) => p.status === 'success' || p.status === 'error').length
)
...
const handleFileSelect = (event) => {
  ...
  uploadProgress.value = validFiles.map((f) => ({
    name: f.name,
    status: 'pending',
    message: '等待上传'
  }))
...
const handleUpload = async () => {
  ...
  for (let i = 0; i < files.value.length; i++) {
    ...
    uploadProgress.value[i].message = '上传中...'

    try {
      const response = await uploadReportFiles([file])
      const result = response?.data || response

      uploadProgress.value[i].status = 'success'
      uploadProgress.value[i].message = '上传成功'

      // 后端返回的 success_reports 包含本次上传成功的记录
      if (result.success_reports) {
        successReports.push(...result.success_reports)
      }
      if (result.failed_files) {
        failedFiles.push(...result.failed_files)
      }
    } catch (error) {
      uploadProgress.value[i].status = 'error'
      uploadProgress.value[i].message = error.message || '上传失败'
      failedFiles.push({
        file_name: file.name,
  ...
  isUploading.value = false
  uploadResult.value = {
    success_count: successReports.length,
    failed_count: failedFiles.length,
    success_reports: successReports,
      ...
      <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-xl rounded-[32px] bg-white shadow-2xl">
          ...
          <div class="p-6">
            ...
            <div
              v-if="!hasFiles"
              class="flex min-h-[200px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-ink-200 bg-ink-50/50 hover:border-ink-300 hover:bg-ink-50"
            ...
            <div v-else class="space-y-4">
              <div class="max-h-[240px] overflow-y-auto rounded-xl border border-black/5">
                <div
              ...
              >
                <p
                  ...
                  :class="uploadResult.failed_count > 0 ? 'text-yellow-700' : 'text-green-700'"
                >
                  上传完成：成功 {{ uploadResult.success_count }} 个
                  <template v-if="uploadResult.failed_count > 0">
                    ，失败 {{ uploadResult.failed_count }} 个
                  </template>
                </p>
              </div>
            </div>
